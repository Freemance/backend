...
type ForwardPaginationArguments = { first: number; after?: number }
type BackwardPaginationArguments = { last: number; before?: number }
...
function mergeDefaultOptions<Record, Cursor, Node, CustomEdge extends Edge<Node>>(
  ...
  return {
    ...
    encodeCursor: (cursor: Cursor) => (cursor as unknown as { id: number }).id,
    decodeCursor: (cursorString: number) => ({ id: cursorString } as unknown as Cursor),
...
function decodeCursor<Record, Cursor, Node, CustomEdge extends Edge<Node>>(
  connectionCursor: number | undefined,
